...
export default function VexFlowQuiz({ lessonType, onQuizComplete }: VexFlowQuizProps) {
    ...
    const loadAbcjs = async () => {
      try {
        ...
        if (containerRef.current) {
          containerRef.current.innerHTML = `<div style="text-align: center; padding: 50px; color: #666;">${lessonType === 'notes' ? '음표' : '쉼표'} 로딩 중...</div>`;
        }
      }
    };
